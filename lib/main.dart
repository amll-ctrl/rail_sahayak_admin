...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      final credential = await FirebaseAuth.instance.signInWithEmailAndPassword(email: _email.text.trim(), password: _password.text);
        ...
        child: Card(child: Padding(
          ...
          child: Form(key: _form, child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            TextFormField(controller: _password, enabled: !_loading, obscureText: _obscure,
              decoration: InputDecoration(labelText: 'Password', prefixIcon: const Icon(Icons.lock_outline), border: const OutlineInputBorder(), suffixIcon: IconButton(icon: Icon(_obscure ? Icons.visibility : Icons.visibility_off), onPressed: () => setState(() => _obscure = !_obscure))),
...
bool _isPendingStaffRequest(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
  final d = doc.data();
...
class AdminDashboard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('RailSahayak Admin'), backgroundColor: RailSahayakAdminApp.red, foregroundColor: Colors.white,
      actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: () => (context as Element).markNeedsBuild()), IconButton(icon: const Icon(Icons.logout), onPressed: () => FirebaseAuth.instance.signOut())]),
    body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (_, requestsSnapshot) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, usersSnapshot) {
          ...
          final staff = users.where((x) => '${x.data()['role']}'.toLowerCase() == 'staff' && !_isPendingStaffRequest(x)).length;
          final pendingStaff = users.where(_isPendingStaffRequest).length;
          ...
          return SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Overview', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)), const SizedBox(height: 20),
...
class PendingStaffScreen extends StatelessWidget {
  ...
  Future<void> _approveRequest(String requestId, String uid, Map<String, dynamic> request) async {
    await FirebaseFirestore.instance.runTransaction((tx) async {
      final userRef = FirebaseFirestore.instance.collection('users').doc(uid);
      final requestRef = FirebaseFirestore.instance.collection('staff_requests').doc(requestId);
      tx.set(userRef, {
        'id': uid,
        'name': request['name'] ?? 'Railway Staff',
        'username': '${request['email'] ?? ''}'.split('@').first.toLowerCase(),
        'email': request['email'] ?? '',
        'phone': request['phone'] ?? '',
        'role': 'staff',
        'status': 'approved',
      ...
      tx.update(requestRef, {'status': 'approved', 'approvedUid': uid, 'approvedAt': FieldValue.serverTimestamp()});
    });
  }

  Future<void> _rejectRequest(String requestId) => FirebaseFirestore.instance.collection('staff_requests').doc(requestId).update({'status': 'rejected', 'rejectedAt': FieldValue.serverTimestamp()});
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${d['name'] ?? 'Staff applicant'}', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6), Text('Email: ${d['email'] ?? '-'}\nPhone: ${d['phone'] ?? '-'}'), const SizedBox(height: 12),
              Row(children: [
                Expanded(child: OutlinedButton.icon(onPressed: () => _rejectRequest(doc.id), icon: const Icon(Icons.close), label: const Text('Reject'), style: OutlinedButton.styleFrom(foregroundColor: Colors.red))),
                const SizedBox(width: 12),
                Expanded(child: ElevatedButton.icon(onPressed: uid.isEmpty ? null : () => _approveRequest(doc.id, uid, d), icon: const Icon(Icons.check), label: const Text('Approve'), style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white))),
              ]),
            ])));
          },
        );
...
class StaffManagementScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _CollectionScreen(
    title: 'Staff Management', stream: FirebaseFirestore.instance.collection('users').where('role', isEqualTo: 'staff').snapshots(), empty: 'No approved staff accounts found.',
    filter: (doc) => !_isPendingStaffRequest(doc),
    itemBuilder: (context, doc) { final d = doc.data(); final disabled = '${d['status']}'.toLowerCase() == 'disabled'; return Card(child: ListTile(
...
class UserManagementScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _CollectionScreen(
    ...
    itemBuilder: (context, doc) { final d = doc.data(); final role = '${d['role'] ?? 'passenger'}'; final disabled = '${d['status']}'.toLowerCase() == 'disabled'; return Card(child: ListTile(
      leading: CircleAvatar(child: Icon(role.toLowerCase() == 'staff' ? Icons.badge : Icons.person)), title: Text('${d['name'] ?? 'User'}'), subtitle: Text('${d['email'] ?? ''}\n$role${disabled ? ' • Disabled' : ''}'), isThreeLine: true,
...
class RequestManagementScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => _CollectionScreen(
    title: filter == null ? 'Assistance Requests' : '${filter![0].toUpperCase()}${filter!.substring(1)} Requests', stream: FirebaseFirestore.instance.collection('requests').snapshots(), empty: 'No assistance requests found.', filter: (doc) => _match('${doc.data()['status'] ?? 'requested'}'),
    itemBuilder: (context, doc) { final d = doc.data(); final status = '${d['status'] ?? 'requested'}'.toLowerCase(); const allowed = ['requested','assigned','assisting','completed','cancelled']; return Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('${d['passengerName'] ?? d['name'] ?? 'Passenger'}', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold)), const SizedBox(height: 6), Text('Train: ${d['trainName'] ?? d['trainNo'] ?? '-'}\nCoach: ${d['coach'] ?? '-'}\nPNR: ${d['pnr'] ?? '-'}\nStatus: $status'), const SizedBox(height: 10),
...
class AdminSettingsScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { final u = FirebaseAuth.instance.currentUser; return Scaffold(appBar: AppBar(title: const Text('Admin Settings')), body: ListView(padding: const EdgeInsets.all(20), children: [
